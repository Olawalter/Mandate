...
/**
 * The write lifecycle. A wallet opening is not success, a hash is not
 * success, an accepted transaction is not a final one, and a receipt is not
 * state: each stage below is only entered when the thing it names has been
 * observed on GenLayer.
 *
 *   READY                  nothing sent
