 ...
 * connected ADDRESS and that wallet's EIP-1193 provider, so genlayer-js hands
 * the transaction to the wallet with `eth_sendTransaction`. No private key
 * exists anywhere in this application, and there is no backend signer.
 */
...
export function walletChain(config: AppConfig) {
  return defineChain({
    ...
    rpcUrls: { default: { http: [config.rpcUrl] } },
    blockExplorers: { default: { name: "GenLayer Explorer", url: config.explorer } },
    testnet: true,
  });
}
