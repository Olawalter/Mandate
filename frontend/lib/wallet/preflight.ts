...
export function preflight(w: WalletSnapshot, config: AppConfig, deploymentOk: boolean | undefined): string | null {
  ...
  if (w.chainId !== config.chainId) {
    return `Your wallet is on another network. Switch it to ${config.networkName} (chain ${config.chainId}) and try again.`;
  }
  if (deploymentOk === false) return "The configured contract is not verified as MANDATE, so no transaction is sent.";
