    ...
    <>
      ...
      >
        ...
        {status === "connecting" || status === "reconnecting" ? (
          <span className="hidden min-[360px]:inline">Connectingâ€¦</span>
        ) : (
        ...
        <DialogContent className="rounded-lg border border-line bg-well sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-dim">
              MANDATE signs with your own browser wallet. It never asks for, sees, or stores a private key.
            </DialogDescription>
          </DialogHeader>
          {listed.length === 0 ? (
            <p className="rounded-md border border-line bg-surface p-4 text-sm text-dim">
            ...
            <ul className="grid gap-2">
              {listed.map((c) => (
                <li key={c.uid}>
                  <button
                    type="button"
                    className="flex w-full items-center gap-3 rounded-md border border-line bg-surface px-4 py-3 text-left text-sm transition-colors hover:border-signal disabled:opacity-50"
