"""Deploy contracts/mandate.py to GenLayer StudioNet and verify the deployment.
...
What it does, in order:
  ...
  2. creates a throwaway deployer account and funds it from the StudioNet
     faucet â€” no private key is requested, printed, or stored, and the
     deployer has no privileges in the contract (MANDATE has no owner or admin);
...
def main() -> int:
    ...
    ap.add_argument("--out", default="docs/deployment-new.json")
    args = ap.parse_args()
    ...
    source = subprocess.run(["git", "show", f"{commit}:contracts/mandate.py"], cwd=ROOT,
                            capture_output=True, check=True).stdout
    digest = hashlib.sha256(source).hexdigest()
    print(f"source    contracts/mandate.py @ {commit[:12]}  {len(source)} bytes  sha256 {digest}")
    ...
    print(f"NEXT_PUBLIC_MANDATE_CONTRACT={address}")
    return 0 if match and info.get("protocol_version", "").startswith("MANDATE") else 1
